...
export class AuthController {
  ...
  constructor() {
    ...
    this.router.post("/login/local/jwt", passport.authenticate(
      "local",
      { session: false }
    ), this.emitJWT);
  }
  ...
  private initPassport() {
    ...
    passport.use(new LocalStrategy(async (username: string, password: string, done: (error: any, user?: User | boolean, options?: IVerifyOptions) => void) => {
      try {
    ...
    }, async (payload: { id: number }, done: (err: any, user: User | false) => void) => {
      try {
