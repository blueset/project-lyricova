...
export default function FileSongInfo({ partialFile, fileId }: Props) {
  ...
  if (query.loading) banner = <Alert severity="info">Loading...</Alert>;
    ...
    banner = <Alert severity="error">Error: {`${query.error}`}</Alert>;
  ...
  const TableRow = useCallback(
    ({ heading, children }: { heading: ReactNode; children: ReactNode }) => (
      <Box sx={{ display: { md: "table-row" } }}>
        ...
        <Typography sx={ResponsiveTableCellSx} variant="body1">
          {children}
        </Typography>
    ...
    <>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item md={file.hasCover ? 8 : 12} xs={12}>
          ...
          <Typography sx={{ lineHeight: 1.2, fontWeight: "bold" }} variant="h5">
            {file.trackName}
          </Typography>
