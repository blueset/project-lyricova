 ...
 * @license MIT
 */
const gradients = [{ "name": "Omolon", "colors": ["#091E3A", "#2F80ED", "#2D9EE0"] }, {
  "name": "Farhan",
...
function initials(source: string): string {
  ...
  else if (words.length === 1) return words[0].substring(0, 2).toLocaleUpperCase();
  else {
...
export default function PlaylistAvatar(props: Props) {
  ...
  const gradient = useMemo(() => gradients[hash(slug) % gradients.length].colors.join(", "), [slug]);
  return (
    <StyledAvatar style={{background: `linear-gradient(135deg, ${gradient})`}} variant="rounded" {...avatarProps}>{initial}</StyledAvatar>
  );
}