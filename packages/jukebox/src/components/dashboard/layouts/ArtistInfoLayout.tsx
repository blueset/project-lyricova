  ...
  if (query.loading) table = <Alert severity="info">Loading...</Alert>;
    ...
    table = <DataGrid 
      columns={[
        ...
        { headerName: "Cover", field: "coverUrl", width: 75, renderCell: ({row: r}) => <Avatar variant="rounded" src={r.mainPictureUrl}>{r.name[0]}</Avatar>},
        // eslint-disable-next-line react/display-name
          ...
          getActions: (rowData) => [
            ...
            <Tooltip title="View in VocaDB" key="ViewInVocaDB" >
              <GridActionsCellItem icon={<OpenInNewIcon/>} label="View in VocaDB"
                ...
                onClick={async () => {
                  if (rowData?.id !== undefined) {
                    window.open(`https://vocadb.net/Ar/${rowData.id}`, "_blank");
                  }
                }} />
            </Tooltip>,
            <Tooltip title="Overwrite from VocaDB" key="OverwriteFromVocaDB" >
              <GridActionsCellItem icon={<GetAppIcon/>} label="Overwrite from VocaDB"
                disabled={(rowData?.id ?? -1) < 0}
                onClick={async () => {
                  if (rowData?.id !== undefined) {
