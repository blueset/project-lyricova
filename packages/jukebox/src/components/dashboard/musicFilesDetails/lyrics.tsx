...
export default function LyricsPanel({fileId, lrcLyrics, lrcxLyrics, refresh, title, artists, duration}: Props) {
  ...
  return (
    ...
    <Grid container spacing={2}>
      ...
      <Grid item xs={12} sm={3}>
        ...
        <p>
          <Typography>Lyrics type: {lrcxLyrics ? <Typography component="span" color="secondary">LRCX</Typography> : lrcLyrics ? "LRC" : <Typography component="span" color="textSecondary">No lyrics</Typography>}</Typography>
          {effectiveLyricsText !== null && (<>
            {lrcxLyrics !== null && <>
              <Typography>Has translation: {analysisResult.hasTranslation ? <Typography component="span" color="secondary">Yes</Typography> : <Typography component="span">No</Typography>}</Typography>
              <Typography>Has inline time tags: {analysisResult.hasInlineTimeTags ? <Typography component="span" color="secondary">Yes</Typography> : <Typography component="span">No</Typography>}</Typography>
              <Typography>Has furigana: {analysisResult.hasFurigana ? <Typography component="span" color="secondary">Yes</Typography> : <Typography component="span">No</Typography>}</Typography>
            </>}
            <Typography>Has “Simplified Japanese”: {analysisResult.hasSimplifiedJapanese ? <Typography component="span" color="error">Yes</Typography> : <Typography component="span" color="secondary">No</Typography>}</Typography>
            <Typography>Last timestamp: {dayjs.utc(analysisResult.lastTimestamp * 1000).format("HH:mm:ss")}</Typography>
          </>)}
        </p>
