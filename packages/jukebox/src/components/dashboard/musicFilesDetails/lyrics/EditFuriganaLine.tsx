...
type RenderableFuriganaElement = string | {
  base: string;
  value: string;
};
...
export default function EditFuriganaLine({ line, setLine }: Props) {
  ...
  const updateLine = useCallback((rf?: RenderableFuriganaElement[]) => {
    const { tags } = (rf || renderableFurigana).reduce<{
      len: number; tags: [string, [number, number]][];
    }>(({ len, tags }, value) => {
      if (typeof value === "string") return { len: len + value.length, tags };
      else {
  ...
  const removeRange = useCallback((idx) => () => {
    const self = renderableFurigana[idx];
    if (typeof self !== "object") return;
    const before = renderableFurigana[idx - 1], after = renderableFurigana[idx + 1];
    let result: RenderableFuriganaElement[];
    if (typeof before === "string" && typeof after === "string") {
      result = renderableFurigana.slice(0, idx - 1).concat([before + self.base + after]).concat(renderableFurigana.slice(idx + 2, renderableFurigana.length));
    } else if (typeof before === "string") {
      result = renderableFurigana.slice(0, idx - 1).concat([before + self.base]).concat(renderableFurigana.slice(idx + 1, renderableFurigana.length));
    } else if (typeof after === "string") {
      result = renderableFurigana.slice(0, idx).concat([self.base + after]).concat(renderableFurigana.slice(idx + 2, renderableFurigana.length));
    } else {
      result = renderableFurigana.slice(0, idx).concat([self.base]).concat(renderableFurigana.slice(idx + 1, renderableFurigana.length));
    }
    setRenderableFurigana(result);
    updateLine(result);
  }, [renderableFurigana, setRenderableFurigana, updateLine]);

  const addFurigana = useCallback((nodeIdx: number, start: number, end: number, value: string) => {
    const node = renderableFurigana[nodeIdx];
    if (typeof node !== "string") return;
    const before = node.substring(0, start), base = node.substring(start, end),
      after = node.substring(end, node.length);
    let result = renderableFurigana.slice(0, nodeIdx);
    if (before) result.push(before);
    result.push({ base, value });
    if (after) result.push(after);
    result = result.concat(renderableFurigana.slice(nodeIdx + 1, renderableFurigana.length));
    setRenderableFurigana(result);
    updateLine(result);
  }, [renderableFurigana, setRenderableFurigana, updateLine]);
  ...
  const onSelect = useCallback(() => {
    ...
    const trigger =
      ...
      (lineContainerRef.current && lineContainerRef.current.contains(selection.anchorNode))
    ;
    if (!trigger) return;
    const show =
  ...
  const handleFloatingInputChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    setFloatingWindowInput(event.target.value);
  }, [setFloatingWindowInput]);

  const handleFloatingInputConfirm = useCallback((e: FormEvent) => {
    e.preventDefault();
    ...
    <div>
      ...
      <Paper style={{ top: floatingWindow.top, left: floatingWindow.left }} className={styles.floatingCard} elevation={5}>
          ...
          <TextField
              ...
              variant="outlined"
          />
          <IconButton type="submit" disabled={!floatingWindowInput}><CheckIcon /></IconButton>
          <IconButton onClick={() => setFloatingWindow(null)}><CloseIcon /></IconButton>
          </form>
      </Paper>}
    </div>
  );
