    ...
    inlineTagging: {
      ...
      setCursorPosition: (position: [number, number]) =>
        set(
          ...
          false,
          "inlineTagging/setCursorPosition",
        ),
      setDotCursorPosition: (position: [number, number, number]) =>
        set(
          ...
          false,
          "inlineTagging/setDotCursorPosition",
        ),
      setCurrentLine(line) {
        set(
          (state) => {
      ...
      moveDotCursorDown() {
        set(
          ...
          false,
          "inlineTagging/moveDotCursorDown",
        );
      },
      moveDotCursorLeft() {
        set(
          ...
          false,
          "inlineTagging/moveDotCursorLeft",
        );
      },
      moveDotCursorRight() {
        set(
          ...
          false,
          "inlineTagging/moveDotCursorRight",
        );
      },
      setDot() {
        set(
      ...
      populateDotsAndMarks() {
        set(
          ...
          false,
          "inlineTagging/populateDotsAndMarks",
        );
      },
      updateTimeTags() {
        set(
          (state) => {
            state.lyrics?.lines.forEach((line, idx, arr) => {
              ...
              const prevTags =
                arr[idx - 1]?.attachments?.[TAGS]?.values?.flat();
              const prevEndTime = prevTags?.length
                ? Math.max(...prevTags)
                : null;
              const nextTags =
                arr[idx + 1]?.attachments?.[TAGS]?.values?.flat();
      ...
      applyMarksToIdenticalLines(index: number) {
        set(
              ...
              if (
                ...
                (nextLine.attachments?.[DOTS]?.values?.length !==
                  ...
                  !(
                    dots?.values?.every(
                      (v, idx) =>
                        v === nextLine.attachments?.[DOTS]?.values[idx],
                    ) ?? false
                  ))
              ) {
          ...
          false,
          "inlineTagging/applyMarksToIdenticalLines",
        );
        get().generate();
      },
      setDots(dots) {
      ...
      setAutoApplyIdentical(value) {
        set(
          ...
          false,
          "inlineTagging/setAutoApplyIdentical",
        );
      },
    },
  };
