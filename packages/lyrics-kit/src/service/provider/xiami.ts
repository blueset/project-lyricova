  ...
  constructor(content: string) {
    ...
    for (const match of lineMatches) {
      ...
      if (match[3] !== undefined) {
        ...
        let lineContent = "";
        const timetagAttachment = new WordTimeTag([new WordTimeTagLabel(0, 0)]);
        let dt = 0.0;
        ...
        for (const m of inlineTagMatches) {
          ...
          dt += timeTag;
          timetagAttachment.tags.push(
            new WordTimeTagLabel(dt, lineContent.length),
          );
        }
...
export class XiamiProvider extends LyricsProvider<XiamiResultSong> {
  ...
  public async searchLyrics(
    ...
    try {
      ...
      const response = await axios.post<XiamiResponseSearchResult>(
        SEARCH_URL,
        "",
