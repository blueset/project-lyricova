...
const token = {
  ...
  Cookie:
    "AWSELB=55578B011601B1EF8BC274C33F9043CA947F99DCFF6AB1B746DBF1E96A6F2B997493EE03F246465F0C13E5B1F720DBC6D3E3802C92E4E48F6DE4DEB9C95CF2A68340C40430",
};
...
export class MusixMatchProvider extends LyricsProvider<MusixMatchEntry> {
  public async searchLyrics(
    ...
    const lyrres = await axios.get<MusixMatchEntry>(
  ...
  public async fetchLyrics(
    token: MusixMatchEntry,
  ): Promise<Lyrics | undefined> {
    const subtitles =
      token?.message?.body?.macro_calls?.["track.subtitles.get"] ?? undefined;
    if (!subtitles) {
      // Data not exist
      return undefined;
    }
    if (
      subtitles?.message?.header?.status_code !== 200 ||
      (subtitles?.message?.body?.subtitle_list?.length ?? 0) === 0
    ) {
      // No lyrics found
      return undefined;
    }
