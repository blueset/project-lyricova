...
class KugouKRCLyrics extends Lyrics {
  constructor(content: string) {
    ...
    const transContent: string[][] | undefined = _.get(
      languageHeader,
      ["content", 0, "lyricContent"],
      undefined,
    );
    if (transContent) {
...
export class KugouProvider extends LyricsProvider<KugouResultItem> {
  ...
  public async searchLyrics(
    ...
    try {
      ...
      const response = await axios.get<KugouResponseSearchResult>(SEARCH_URL, {
        params: parameter,
      });
